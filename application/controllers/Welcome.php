...
class Welcome extends CI_Controller {
	...
	{	
	    ...
	    $masuk = $this->db->query('select * from tab_absensi where tgl_kerja>="'.$tgl1.'" and tgl_kerja<="'.$tgl2.'" and status_masuk="Masuk" or tgl_kerja>="'.$tgl1.'" and tgl_kerja<="'.$tgl2.'" and status_masuk2="Masuk"')->num_rows();
	    $cuti = $this->db->query('select * from tab_absensi where tgl_kerja>="'.$tgl1.'" and tgl_kerja<="'.$tgl2.'" and status_masuk="Cuti" or tgl_kerja>="'.$tgl1.'" and tgl_kerja<="'.$tgl2.'" and status_masuk2="Cuti"')->num_rows();
	    $izin = $this->db->query('select * from tab_absensi where tgl_kerja>="'.$tgl1.'" and tgl_kerja<="'.$tgl2.'" and status_masuk="Izin" or tgl_kerja>="'.$tgl1.'" and tgl_kerja<="'.$tgl2.'" and status_masuk2="Izin"')->num_rows();
	    $absen = $this->db->query('select * from tab_absensi where tgl_kerja>="'.$tgl1.'" and tgl_kerja<="'.$tgl2.'" and status_masuk="Bolos" or tgl_kerja>="'.$tgl1.'" and tgl_kerja<="'.$tgl2.'" and status_masuk2="Bolos"')->num_rows();
	    $absen2 = $this->db->query('select * from tab_absensi INNER JOIN tab_resign ON tab_resign.nik=tab_absensi.nik where tgl_kerja>="'.$tgl1.'" and tgl_kerja<="'.$tgl2.'" and status_masuk="Bolos" or tgl_kerja>="'.$tgl1.'" and tgl_kerja<="'.$tgl2.'" and status_masuk2="Bolos"')->num_rows();

	    $data['masuk']=json_encode($masuk);
	    $data['karyawan']=json_encode($karyawan);
	    $data['cuti']=json_encode($cuti);
	    $data['izin']=json_encode($izin);
	    $data['absen']=json_encode($absen-$absen2);
		...
		$data['data_resign']=$this->model_dashboard->notifikasi_resign(date('Y-m-01'),date('Y-m-t'));
		$data['data_schedule']=$this->model_dashboard->schedule_pindah($t1="");

		$data['data_bpjs1']=$this->model_dashboard->bpjs_kesehatan($tgl1,$tgl2);
		$data['data_bpjs2']=$this->model_dashboard->bpjs_ketenagakerjaan($tgl1,$tgl2);
	...
	{
		$data = $this->model_dashboard->notifikasi_resign($this->input->post('tgl1'),$this->input->post('tgl2'));

		echo '<table id="example-4" class="table table-hover table-striped table-bordered">
	    		<thead>
		    		<tr class="bg-warning-700 color-white">
		    			<th>NO</th>
		    			<th>NIK</th>
		...
		foreach ($data as $view_res) {
			...
			if ($cek<1) {
				//echo "<tr><td>$no</td><td>$view_res->nik</td><td>$view_res->nama_ktp</td><td>$view_res->status_kerja</td><td>$view_res->cabang</td><td>$tgl_masuk</td><td>$tgl_resign</td><td><a href='KaryawanController/profil/$view_res->nik'><span class='label label-info'>View Detail</span></a>|<a href='javascript:void(0)' onclick=delNotif(this,$view_res->id_kon,'resign')><label class='label label-danger'>Delete</label></a></td></tr>";
	...
	public function cari_jadwal()
	{
		...
		foreach ($data as $rs) {
			...
			echo "<tr><td>$no</td><td>$rs->nik</td><td>$rs->nama_ktp</td><td>$rs->cabang</td><td>$rs->kode_jam_asal</td><td>$rs->kode_jam_pindah</td><td>$rs->keterangan</td><td>$tgl_sc</td></tr>";
			$no++;
		}
	}
	...
	public function cari_bpjs_kesehatan()
	{
		$tgl1 = $this->input->post('tgl1',true);
		$tgl2 = $this->input->post('tgl2',true); 
		$data = $this->model_dashboard->bpjs_kesehatan($tgl1,$tgl2);
	...
	public function cari_bpjs_tenaga()
	{
		$tgl1 = $this->input->post('tgl1',true);
		$tgl2 = $this->input->post('tgl2',true); 
		$data = $this->model_dashboard->bpjs_ketenagakerjaan($tgl1,$tgl2);
		echo '<table id="example-7" class="table table-hover table-striped table-bordered">
		    		<thead>
			    		<tr class="bg-warning-700 color-white">
			    			<th>NO</th>
			    			<th>NIK</th>
