			...
			<tbody>
				...
				$no=1;
					foreach ($data as $rs) {
						echo "<tr>
								<td>$no</td>
								<td hidden><input type='hidden' name='nik[]' value='$rs->nik' /></td>
								<td>$rs->nik</td>
								<td>$rs->nama_ktp</td>
								<td>$rs->jabatan</td>
								<td><input class='form-control' type='text' name=tarif[]  data-mask='#.##0' data-mask-reverse='true' data-mask-maxlength='false'/></td>
								</tr>";
					$no++;
