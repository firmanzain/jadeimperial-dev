...
<div class="form-group" style="margin-bottom: 20px;">
	<p>Yth.<br> 
	BPJS Ketenagakerjaan KCU Surabaya</p>
</div>
<div class="form-group">
	...
	<table width="100%" class="tabel">
		...
		<?php
			...
			foreach ($karyawan as $rs) {
				$no_bpjs=$this->db->where('nik',$rs->nik)->where('id_bpjs',1)->select('no_bpjs')->get('tab_bpjs_karyawan')->row();
				echo "<tr><td>$no</td><td>$no_bpjs->no_bpjs</td><td>$rs->nama_ktp</td><td>$rs->nik</td><td>".date('d-M-Y',strtotime($rs->tanggal_lahir))."</td></tr>";
				$no++;
			}
		?>
	</table>
	<p style="text-align: justify;">Kami menjamin data yang diserahkan adalah lengkap dan benar serta bertanggung jawab sepenuhnya terhadap kebenaran data. BPJS Ketenagakerjaan berhak memproses data yang telah lengkap dan sesuai ketentuan tertib administrasi kepesertaan. Data yang tidak sesuai tidak dapat diproses
