...
class Bonus extends CI_Model {
    ...
    function detail($id,$bln,$thn){
        ...
        $this->db->select('a.*,a.bonus_point as bonus_point1,a.nik as nik_bonus,b.*,c.*,d.*,f.*,g.prota,c.cabang as nama_cabang');
        ...
        $this->db->where('c.id_cabang',$id);
        $this->db->where('month(a.tanggal_bonus)',$bln);
        $this->db->where('year(a.tanggal_bonus)',$thn);
        $baca = $this->db->get();
        if ($baca->num_rows() > 0){
            return $baca->result();
        }else{
